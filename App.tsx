    ...
    <div className={`min-h-screen ${solarMode ? 'bg-white text-black' : 'bg-[#020202] text-zinc-100'} pb-24 font-sans relative`}>
      <header className="max-w-4xl mx-auto pt-10 px-4 mb-12">
        <div className="flex justify-between items-center mb-4">
           <button onClick={()=>window.location.reload()} className="px-4 py-2 border-2 border-zinc-800 rounded-full font-black uppercase text-[9px] text-zinc-600 transition-all hover:text-white">Clear All</button>
      ...
      <div className="max-w-4xl mx-auto space-y-8 px-4">
        <section className={`bg-zinc-900/40 border-2 rounded-[2.5rem] p-8 border-zinc-800`} style={{ borderColor: (company && driverName) ? themeHex : '' }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            {!manualDriver ? (
              <select className={getTacticalStyles(driverName)} value={driverName} onChange={(e)=>{ if(e.target.value === "ADD_MANUAL") setManualDriver(true); else setDriverName(e.target.value); }}><option value="">SELECT DRIVER</option>{driverList.map(d=><option key={d} value={d}>{d}</option>)}<option value="ADD_MANUAL">+ TYPE NAME MANUALLY</option></select>
            ) : (
              <input type="text" placeholder="TYPE FULL NAME" className={getTacticalStyles(driverName)} value={driverName} onChange={(e)=>setDriverName(e.target.value.toUpperCase())} autoFocus />
            )}
          </div>
        </section>
        ...
        <button onClick={()=>isReady && setShowVerification(true)} className={`w-full py-10 rounded-[2.5rem] font-black uppercase tracking-[1.5em] border-[3px] border-white transition-all duration-700 ${isReady ? 'bg-blue-600 text-white shadow-[0_0_80px_rgba(59,130,246,0.3)] scale-[1.02]' : 'bg-zinc-900 text-zinc-700 opacity-50'}`}>{isReady ? 'REVIEW DOCUMENTS' : 'COMPLETE FIELDS'}</button>
      </div>

      {showFreightPrompt && (
        <div className="fixed inset-0 z-[200] bg-black/90 flex items-center justify-center p-6 animate-in fade-in">
          <div className="bg-zinc-900 border-2 border-blue-500 rounded-[2.5rem] p-10 text-center">
            <h2 className="text-xl font-black text-blue-500 uppercase mb-4">Pickup Detected</h2>
            ...
            <div className="flex flex-col gap-4">
              <button onClick={()=>{ setShowFreightPrompt(false); freightCamRef.current?.click(); }} className="bg-blue-600 text-white py-4 rounded-xl font-black uppercase tracking-widest shadow-xl">Yes, Open Camera</button>
      ...
      {showVerification && (
        <div className="fixed inset-0 z-[400] bg-black flex flex-col items-center justify-center p-6">
          <div className="w-full max-w-lg bg-zinc-900 border-2 rounded-[3.5rem] p-10 border-blue-500 relative">
             ...
             <div className="space-y-4 mb-12 font-mono text-sm text-white uppercase">
                ...
                <div className="flex justify-between border-b border-zinc-800 pb-2 text-blue-400"><span>Load #</span><span>{loadNum}</span></div>
             ...
             <button onClick={()=>setShowVerification(false)} className="w-full mt-4 text-zinc-600 font-black uppercase text-[10px] tracking-widest">Back to Fix</button>
          </div>
        </div>
      )}
