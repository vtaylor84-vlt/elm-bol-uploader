...
/** * LOGISTICS TERMINAL v31.9
 * - RESTORED: Full v31.8 Tactical UI & Logos.
 * - FIXED: Dynamic Driver Dropdown (Alphabetical).
 * - ADDED: Connection Status & Vault Alerts.
 */
...
export const BstLogo: React.FC = () => (
  <div className="flex flex-col items-center justify-center p-4 w-full min-h-[180px] animate-in fade-in duration-700"> 
    <svg width="320" height="120" viewBox="0 0 400 120" xmlns="http://www.w3.org/2000/svg">
      ...
      <text x="200" y="75" textAnchor="middle" style={{ fontSize: '95px', fill: 'url(#bst-metal)', fontFamily: 'Arial Black, sans-serif', fontWeight: '900', fontStyle: 'italic' }}>BST</text>
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetchDrivers();
    const refreshVault = () => { setVaultEntries(JSON.parse(localStorage.getItem('multi_vault') || '[]')); };
    refreshVault();
  ...
  if (isLocked) return (
    <div className="min-h-screen bg-black flex flex-col items-center justify-center p-6 text-white font-sans relative">
      <div className="absolute top-6 right-6 flex items-center gap-2 px-4 py-2 bg-zinc-900 rounded-full border border-zinc-800">
        <div className={`w-2 h-2 rounded-full ${GOOGLE_SCRIPT_URL ? 'bg-green-500 shadow-[0_0_8px_#22c55e]' : 'bg-red-500'}`}></div>
        <span className="text-[9px] font-black uppercase tracking-widest text-zinc-400">Uplink: {GOOGLE_SCRIPT_URL ? 'Online' : 'Offline'}</span>
      </div>
      <button onClick={() => { let s=0; const inv=setInterval(()=>{ s++; setAuthStage(s); if(s>=4){ clearInterval(inv); setIsLocked(false); }},600); }} className="w-48 h-48 border-4 border-blue-500/30 rounded-full flex flex-col items-center justify-center bg-zinc-950 shadow-2xl active:scale-90">
        <span className="text-6xl mb-2">🛡️</span>
        <span className="text-[10px] font-black text-blue-400 tracking-widest animate-pulse">INITIALIZE</span>
      </button>
    ...
    <div className={`min-h-screen ${solarMode ? 'bg-white text-black' : 'bg-[#020202] text-zinc-100'} pb-24 font-sans relative`}>
      <header className="max-w-4xl mx-auto pt-10 px-4 mb-12">
        <div className="flex justify-between items-center mb-4">
           <button onClick={()=>window.location.reload()} className="px-4 py-2 border-2 border-zinc-800 rounded-full font-black uppercase text-[9px] tracking-widest text-zinc-500">Clear All</button>
      ...
      <div className="max-w-4xl mx-auto space-y-8 px-4">
        ...
        {/* ... References & Route sections remain identical to v31.8 ... */}
        ...
        <section className="bg-zinc-900/40 border-2 rounded-[2.5rem] p-8 border-zinc-800" style={{ borderColor: bolProtocol ? themeHex : '' }}>
          ...
          {uploadedFiles.length >= 5 && <p className="text-center text-[10px] font-black text-orange-500 animate-pulse mb-4">⚠️ LARGE SCAN ({uploadedFiles.length} PAGES)</p>}
        ...
        <button onClick={()=>{ if(isReady) setShowVerification(true); }} className={`w-full py-10 rounded-[2.5rem] font-black uppercase tracking-[1.5em] border-[3px] border-white transition-all ${isReady ? 'bg-blue-600 text-white shadow-2xl scale-[1.02]' : 'bg-zinc-900 text-zinc-700 opacity-50'}`}>
          {isReady ? 'REVIEW' : 'COMPLETE'}
        </button>
        ...
        {vaultEntries.length > 0 && (
          <section className="bg-red-900 border-4 border-white rounded-[2.5rem] p-8 mt-12 animate-pulse text-center">
             ...
             <button onClick={async ()=>{ setIsSyncing(true); const rem=[...vaultEntries]; for(let i=0; i<vaultEntries.length; i++){ await fetch(GOOGLE_SCRIPT_URL,{method:'POST',mode:'no-cors',body:JSON.stringify(vaultEntries[i].payload)}); rem.shift(); localStorage.setItem('multi_vault', JSON.stringify(rem)); setVaultEntries([...rem]); } setIsSyncing(false); }} className="w-full py-6 rounded-2xl font-black uppercase bg-white text-red-600">{isSyncing ? 'SYNCING...' : 'Push to Dispatch'}</button>
          </section>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-[400] bg-black flex flex-col items-center justify-center p-6">
          <div className="w-full max-w-lg bg-zinc-900 border-2 rounded-[3.5rem] p-10 border-blue-500">
             ...
             <button onClick={async ()=>{ 
                ...
                catch(e){ const v=[...vaultEntries,{id:Math.random().toString(36).substr(2,9),payload}]; localStorage.setItem('multi_vault', JSON.stringify(v)); setShowSuccess(true); } 
             }} className="w-full py-8 bg-[#ccff00] text-black rounded-[1.5rem] font-black uppercase tracking-[0.4em]">{isSubmitting ? 'SENDING...' : 'Transmit'}</button>
          </div>
        </div>
      )}
      {showSuccess && <div className="fixed inset-0 z-[500] bg-black flex items-center justify-center text-white text-4xl font-black animate-bounce" onClick={()=>window.location.reload()}>✓ SENT</div>}
      <input type="file" ref={cameraInputRef} className="hidden" capture="environment" accept="image/*" multiple onChange={async (e)=>{ if(e.target.files){ const files=Array.from(e.target.files); for(const f of files){ const enh=await compressAndEnhanceImage(f); setUploadedFiles(prev=>[...prev,{file:enh,preview:URL.createObjectURL(enh),id:Math.random().toString(),category:'bol'}]); }} }} />
    </div>
  );
};
export default App;